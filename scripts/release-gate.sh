...
if [ "$version" != "2.0.12" ]; then
  printf '%s
' "FORGE  expected version 2.0.12, got $version" >&2
  exit 1
fi
...
if command -v sha256sum >/dev/null 2>&1; then
  (cd "$package_dir" && sha256sum forge-2.0.12-* > SHA256SUMS)
elif command -v shasum >/dev/null 2>&1; then
  (cd "$package_dir" && shasum -a 256 forge-2.0.12-* > SHA256SUMS)
else
  printf '%s
