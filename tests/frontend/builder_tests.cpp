...
int main() {
    ...
    builder.set_source_range("sample.ds", 3, 7, 3, 18);
    ...
    if (entry.operations.front().source_file != "sample.ds" || entry.operations.front().source_line != 3 ||
    ...
    if (builder.module_metadata("frontend.language") != "Dash") return 1;
    ...
    try { (void)builder.create_constant(forge::ir::i64_type(), "1"); }
        ...
        rejected_after_terminator = std::string(error.what()).find("sample.ds:3:7") != std::string::npos;
    }
    ...
    if (source_map.find("\"frontend.language\":\"Dash\"") == std::string::npos ||
    ...
    if (manifest.find("\"semanticFingerprint\"") == std::string::npos ||
        ...
        return 1;
    const auto cache_key = forge::ir::build_cache_key(module, "dash", "-O2;x86_64");
    if (cache_key.size() != 64 || cache_key != forge::ir::build_cache_key(module, "dash", "-O2;x86_64") ||
        cache_key == forge::ir::build_cache_key(module, "dash", "-O3;x86_64"))
        return 1;
