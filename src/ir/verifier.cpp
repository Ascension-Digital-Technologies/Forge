...
Diagnostics verify_module(const Module& module) {
    ...
    for (const auto& array : module.arrays()) {
        ...
        if (array.element_count == 0 || !data_layout.array_layout(array).has_value())
    ...
    for (const auto& global : module.globals()) {
        ...
        if (global.alignment != 0 && (global.alignment > 16 || (global.alignment & (global.alignment - 1U)) != 0))
            diagnostics.push_back({DiagnosticSeverity::error, "global @" + global.name + " requires power-of-two alignment no greater than 16", {}});
    ...
    for (const auto& function : module.functions()) {
        ...
        while (dom_changed) {
            ...
            for (std::size_t i = 1; i < block_count; ++i) {
                ...
                std::unordered_set<std::size_t> next = dominators[predecessors[i][0]];
                for (std::size_t p = 1; p < predecessors[i].size(); ++p) {
        ...
        for (std::size_t block = 0; block < block_count; ++block) {
            ...
            for (std::size_t operation = 0; operation < function.blocks[block].operations.size(); ++operation) {
                ...
                } else if (op.opcode == "global.address" && !op.operands.empty() && op.operands[0].starts_with("@")) {
        ...
        for (std::size_t block = 0; block < block_count; ++block) {
            ...
            auto verify_use = [&](const std::string& value, std::size_t operation_index) {
                ...
                if (found == definitions.end()) {
                    diagnostics.push_back({DiagnosticSeverity::error, "use of undefined value " + value, {}});
                    return;
                }
            ...
            for (std::size_t operation_index = 0; operation_index < current.operations.size(); ++operation_index) {
                        ...
                        if (signature == function_table.end()) {
                            ...
                            for (std::size_t i = 0; i < count; ++i) {
                                ...
                                if (found->second.type != target.parameters[i].type)
                                    diagnostics.push_back({DiagnosticSeverity::error, "indirect call argument type mismatch for signature " + operation.operands[1], {}});
                    ...
                    if (operation.operands.empty() || !operation.operands[0].starts_with("@")) {
                        ...
                        if (callee == function_table.end()) {
                            ...
                            for (std::size_t i = 0; i < count; ++i) {
                                ...
                                if (found != definitions.end()) {
                                    if (found->second.type != target.parameters[i].type)
                                        diagnostics.push_back({DiagnosticSeverity::error, "call argument type mismatch for " + operation.operands[0], {}});
                                    else if (target.parameters[i].is_aggregate() &&
                                             (found->second.aggregate_kind != target.parameters[i].aggregate_kind ||
                                              found->second.aggregate_name != target.parameters[i].aggregate_name))
                ...
                if (operation.alignment != 0) {
                    const bool allowed = operation.opcode == "stack.alloc" || operation.opcode == "load" || operation.opcode == "store";
                    const bool power_of_two = (operation.alignment & (operation.alignment - 1U)) == 0;
                    if (!allowed) {
                        diagnostics.push_back({DiagnosticSeverity::error,
                            "alignment is only valid on stack.alloc, load, or store in block " + current.name, {}});
                    } else if (!power_of_two || operation.alignment > 16U) {
                        diagnostics.push_back({DiagnosticSeverity::error,
                ...
                if (operation.opcode == "array.element.address") {
                    ...
                    if (!valid) diagnostics.push_back({DiagnosticSeverity::error,
                        "array.element.address requires a ptr base, known array, valid element index, and ptr result in block " + current.name, {}});
                }
