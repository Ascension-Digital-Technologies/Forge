...
private:
    ...
    std::vector<std::uint8_t> parse_array_initializer(const ArrayDecl& declaration) {
        const auto layout = target::DataLayout::host().array_layout(declaration);
        ...
        for (std::uint32_t index = 0; index < declaration.element_count; ++index) {
            ...
            write_scalar(bytes, static_cast<std::size_t>(index) * layout->stride, declaration.element_type,
                         parse_integer_bits(declaration.element_type));
    ...
    std::vector<std::uint8_t> parse_struct_initializer(const Module& module, const StructDecl& declaration) {
        ...
        while (!at("}")) {
            ...
            if (found->aggregate_kind == AggregateRefKind::structure) {
                ...
                field_bytes = parse_array_initializer(*nested);
            } else {
    ...
    Global parse_global(const Module& module) {
        ...
        expect(":");
        if (take("callback")) {
            global.function_signature_name = strip(expect_kind(TokenKind::symbol, "callback signature").text);
        ...
        if (!global.is_external) {
            ...
            if (take("zero")) global.zero_initialized = true;
                ...
                global.bytes = parse_array_initializer(*declaration);
    ...
    Operation parse_operation() {
        ...
        if (op.opcode == "const" || op.opcode == "load" || op.opcode == "neg" || op.opcode == "not" || op.opcode == "copy" || op.opcode == "stack.alloc" ||
            op.opcode == "truncate" || op.opcode == "zero_extend" || op.opcode == "sign_extend" || op.opcode == "bitcast" || op.opcode == "global.address" ||
        ...
        else if (op.opcode == "struct.field.address" || op.opcode == "struct.field.name.address" || op.opcode == "array.element.address" || op.opcode == "callback.element.address" || op.opcode == "memory.copy" || op.opcode == "memory.set") arity = 3;
