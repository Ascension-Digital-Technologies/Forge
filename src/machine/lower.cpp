...
bool parse_float_bits(std::string_view text, bool wide, std::int64_t& bits) {
    ...
    char* tail = nullptr;
    const double parsed = std::strtod(owned.c_str(), &tail);
...
LowerResult lower_module(const ir::Module& source) {
    ...
    for (const auto& array : source.arrays()) {
        const auto layout = host_layout.array_layout(array);
    ...
    for (const auto& global : source.globals()) {
        ...
        output.globals.push_back({global.name, size, alignment, global.is_constant, global.is_external, std::move(bytes)});
    }
            ...
            const auto append_control_successors = [&](const ir::Operation& operation, Instruction& target_instruction) -> bool {
                for (std::size_t successor_index = 0; successor_index < operation.successors.size(); ++successor_index) {
                    ...
                    for (std::size_t argument_index = 0; argument_index < operation.successor_arguments[successor_index].size(); ++argument_index) {
                        ...
                        if (const auto found = registers.find(argument); found != registers.end()) source = found->second;
                        else if (const auto address = addresses.find(argument); address != addresses.end()) {
                            source = allocate_register(ir::Type(ir::TypeKind::ptr));
                            machine_block.instructions.push_back({Opcode::load_stack_address, source, {},
                        ...
                        if (argument_index < target_it->parameters.size() && target_it->parameters[argument_index].owned) {
                            ...
                            machine_block.instructions.push_back({Opcode::call_void, 0, {destination, source, count}, 0, 0, "__forge_memmove", {}});
                            source = destination;
                        }
                        successor.arguments.push_back(source);
                    }
                ...
                if (operation.opcode == "return") {
                    if (function.return_owned) {
                        ...
                        if (resolved == size_table.end() || (source_register == registers.end() && source_address == addresses.end()) ||
                            ...
                            VirtualRegister source{};
                            if (source_register != registers.end()) source = source_register->second;
                            else {
                                source = allocate_register(ir::Type(ir::TypeKind::ptr));
                    ...
                    if (operation.operands.empty() || !operation.operands.front().starts_with("@")) {
                        ...
                        if (!operation.result.empty()) {
                            ...
                            const auto target_it = function_table.find(instruction.symbol);
                            failed = !append_call_arguments(instruction, target_it == function_table.end() ? nullptr : target_it->second);
                        }
                    } else if (operation.result.empty() || (operation.type != ir::Type(ir::TypeKind::i32) && operation.type != ir::Type(ir::TypeKind::i64) && operation.type != ir::Type(ir::TypeKind::ptr) && operation.type != ir::Type(ir::TypeKind::f32) && operation.type != ir::Type(ir::TypeKind::f64))) {
                    ...
                    const bool dynamic_callback = operation.opcode == "callback.element.address" && operation.operands.size() == 3 && operation.operands[2].starts_with('%') &&
                                                  registers.contains(operation.result);
                    if (dynamic_callback) {
                        // The checked helper call above fully materializes the result.
                    ...
                    const auto access_size = operation.type == ir::Type(ir::TypeKind::i8) ? 1U :
                    ...
                    } else if (const auto source = registers.find(operation.operands[0]); source != registers.end()) {
                        registers.emplace(operation.result, source->second);
                        emit = false;
                    } else if (const auto source = addresses.find(operation.operands[0]); source != addresses.end()) {
                        addresses.emplace(operation.result, source->second);
                        emit = false;
                    } else {
                    ...
                    const auto source = copy && operation.operands.size() > 1 ? materialize_pointer(operation.operands[1]) : std::optional<VirtualRegister>{};
                    if (operation.type != ir::Type(ir::TypeKind::void_) || !operation.result.empty() || size_it == sizes.end() || !destination || (copy && !source)) {
                        ...
                        instruction.inputs.push_back(*destination);
                        if (copy) instruction.inputs.push_back(*source);
                        else {
                    ...
                    const auto value = operation.operands.size() == 2 ? registers.find(operation.operands[0]) : registers.end();
                    ...
                    const auto access_size = operation.type == ir::Type(ir::TypeKind::i8) ? 1U :
                                             ...
                                             (operation.type == ir::Type(ir::TypeKind::i64) || operation.type == ir::Type(ir::TypeKind::ptr) || operation.type == ir::Type(ir::TypeKind::f64)) ? 8U :
                                             operation.type == ir::Type(ir::TypeKind::f32) ? 4U : 0U;
                    if (access_size == 0 || value == registers.end() || operation.operands.size() != 2) {
                        ...
                        if (address != addresses.end()) {
                            ...
                            else { instruction.opcode = stack_opcode; instruction.inputs.push_back(value->second); instruction.immediate = -static_cast<std::int64_t>(address->second.object_base + address->second.object_size - address->second.offset); }
                        } else {
                            ...
                            else { instruction.opcode = pointer_opcode; instruction.inputs = {value->second, pointer->second}; }
                        }
                    }
                } else {
                    if (operation.result.empty()) {
                    ...
                    if (operation.opcode == "const") {
                        ...
                        if (operation.operands.size() != 1 || !(float_value ? parse_float_bits(operation.operands[0], wide_float, instruction.immediate) : (wide_value ? parse_i64(operation.operands[0], instruction.immediate) : parse_i32(operation.operands[0], instruction.immediate)))) {
                        ...
                        const auto source = operation.operands.empty() ? value_types.end() : value_types.find(operation.operands[0]);
                        if (source == value_types.end() || !source->second.is_integer() || !operation.type.is_integer()) {
