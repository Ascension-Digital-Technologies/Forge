...
ElfObjectResult emit_elf64_x86_64_image(codegen::x86_64::EncodedModuleImage image) {
    ...
    // Section indexes are fixed by this order.
    constexpr std::uint16_t text_index = 1;
    constexpr std::uint16_t rodata_index = 2;
    constexpr std::uint16_t data_index = 3;
    constexpr std::uint16_t rela_text_index = 4;
    constexpr std::uint16_t symtab_index = 5;
    constexpr std::uint16_t strtab_index = 6;
    constexpr std::uint16_t note_stack_index = 7;
    constexpr std::uint16_t shstrtab_index = 8;
    ...
    constexpr std::uint32_t first_global_symbol = 4;
    ...
    for (const auto& global : encoded.image.globals) {
        const auto section = global.section == codegen::x86_64::DataSection::read_only ? rodata_index : data_index;
        const auto& bytes = global.section == codegen::x86_64::DataSection::read_only
            ? encoded.image.read_only_data : encoded.image.writable_data;
        ...
        symbols.push_back({strings.add(global.name), static_cast<std::uint8_t>((stb_global << 4U) | stt_object), 0,
