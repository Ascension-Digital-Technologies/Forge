...
CoffObjectResult emit_coff_x86_64_image(codegen::x86_64::EncodedModuleImage image) {
    ...
    for (const auto& global : encoded.image.globals) {
        ...
        if (!add_defined_symbol(global.name, value,
                   global.section == codegen::x86_64::DataSection::read_only ? 2 : 3, 0)) return result;
    }
    ...
    auto push_relocation = [&](std::size_t offset, std::uint32_t symbol) -> bool {
        std::uint32_t converted{};
        if (!checked_u32(offset, converted)) return false;
        text_relocations.push_back({converted, symbol});
        return true;
    };
    ...
    for (const auto& section : sections) {
        ...
        for (const auto& relocation : section.relocations) {
            ...
            writer.integer(reloc_amd64_rel32);
        }
    }
