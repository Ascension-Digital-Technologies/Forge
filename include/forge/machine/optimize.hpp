...
struct SlpCostModel {
    ...
    // Hardware capability and backend capability are deliberately separate.
    // The x86-64 packed integer encoder now supports XMM/SSE2, YMM/AVX2, and
    // ZMM/AVX-512. Keeping these fields separate still prevents future target
    // profiles from advertising widths the selected backend cannot emit.
    ...
    // separate so a target profile can distinguish deep dependency chains from
...
struct OptimizationStats {
    ...
    std::uint32_t loop_vector_candidates_considered{};
    std::uint32_t loop_vector_candidates_selected{};
    std::uint32_t loop_vector_candidates_rejected_target{};
    std::uint32_t loop_vector_candidates_rejected_cost{};
    std::uint32_t loop_vector_width_128_selected{};
    std::uint32_t loop_vector_width_256_selected{};
    std::uint32_t loop_vector_width_512_selected{};
