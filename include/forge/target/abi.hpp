...
enum class AbiValueClass : std::uint8_t { none, integer, sse, memory, indirect };
...
struct AggregateAbiClassification {
    ...
    [[nodiscard]] bool register_passed() const noexcept {
        return valid() && !passed_indirectly && register_count != 0;
    }
};
...
[[nodiscard]] std::optional<AggregateAbiClassification> classify_aggregate(
    ...
    NativeAbi abi,
    const DataLayout& layout = DataLayout::host()) noexcept;
